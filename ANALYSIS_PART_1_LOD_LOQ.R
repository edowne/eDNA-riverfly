...
# Data Analysis Part 1: qPCR analysis - Melt curves & LOD/LOQ ####
...
# Reading in dataset 
data_all_wells <- read.csv("WellResult_AllPlates.csv")

# Checking dataset
summary(data_all_wells)
str(data_all_wells)

# Plotting the standard curve
plot_stdcurve <- ggplot(data_all_wells, aes(x=Quantity, y=Cq)) +
  geom_point(colour="#228886")+
  theme_bw()+
  scale_x_log10()+
  geom_smooth(method = "loess", se = FALSE, color = "#C45A12") +
  labs(x="Relative quantity of target DNA", y="Cq value")

plot_stdcurve


# Analysing melt curves:

# Sub-setting the data for Tm values: 
data_std_curve_Tm <- data_all_wells %>% dplyr::select(Sample, Quantity, Target, Cq, Tm1, Tm2, Tm3, Tm_Target)# %>%
  #filter(str_starts(Sample, "STD") | str_starts(Sample, "NTC"))

# Melt the data into long format
data_std_curve_Tm <- pivot_longer(data_std_curve_Tm, cols = starts_with("Tm"), 
                                  names_to = "Peak", values_to = "Tm")

# Remove NAs from Tm column
data_std_curve_Tm <- data_std_curve_Tm %>% drop_na(Tm)

# Filter to show all individual peaks detected 
data_std_curve_Tm_All <- data_std_curve_Tm %>% filter(Peak=="Tm1" | Peak=="Tm2" | Peak=="Tm3")

# Plot melt peaks
plot_melts <- ggplot(data_std_curve_Tm_All, aes(x=Cq, y=Tm))+
  geom_point(colour="#228886")+
  geom_hline(yintercept = 75, linetype="dashed", colour="#C45A12")+
  xlab("Cq value")+
  ylab("Melt peak temperature (°C)")+
  annotate("text", x=22.5, y=75.4, label="Target amplification threshold applied (>75°C)", 
           colour="#C45A12", size=3.1)+
  theme_bw()

plot_melts

# If melt curve <75'C, assume this signifies non-specific products or primer 
# dimers, therefore can be considered a non-detection of the target (hoglouse) DNA. 
# Therefore, Cq values for any sample that has a Tm < 75'C can be revised to NA. 

# Use target Tm values over dimers: 
data_std_curve_Tm_Target <- data_std_curve_Tm %>% filter(Peak=="Tm_Target")

# Convert Cq values for Tm<75'C to NA: 
data_std_curve_Tm_Cq_adjust <- data_std_curve_Tm_Target %>%
  mutate(Cq = if_else(Tm < 75, NA_real_, Cq))



# LOD & LOQ ####

# Calculating LOD and LOQ following code from Merkes et al (2019). Other than the
# initial lines to rename items to match the code in the paper, the rest of the 
# code in this script is directly pasted from that paper. 

# Focus dataset only on standard wells: 
data_std_curve_Tm_Cq_adjust <- data_std_curve_Tm_Cq_adjust %>% 
  filter(str_starts(Sample, "STD"))

# Re-name data file to match code: 
DAT <- data_std_curve_Tm_Cq_adjust

# Re-name "Quantity" column to "SQ" to match code: 
DAT$SQ <- DAT$Quantity


## Define CV threshold for LoQ:
LOQ.Threshold <- 0.35
...
# Re-running without outlier ####

# Having run the LOD and LOQ calculations once, one outlier has been identified
# under STD-1 at Cq value 24.487333. I will now remove this outlier and re-run the model. 

# Remove the row where Sample is "STD-1" and Cq is 24.487333
DAT <- DAT[!(DAT$Sample == "STD-1" & DAT$Cq == 24.487333), ]

# Re-run entire code for LOD/LOQ: 
...
# I ADDED THESE LINES TO EXTRACT THE X VALUES OF LOD AND LOQ:
# Extract LOD and LOQ values for the current target
current_LOD <- DAT3$LOD[i]
current_LOQ <- DAT3$LOQ[i]

# Print the extracted values
cat(sprintf("For target %d: LOD = %.2f, LOQ = %.2f\n", i, current_LOD, current_LOQ))

# What is y when x = LOD?
Cq_LOD <- Slope*current_LOD + Intercept
Cq_LOD

# Now back to the Merkes et al (2019) code:
...
# Filter dataset to show only standards:
data_std_curve_model <- data_all_wells %>% dplyr::select(Sample, Quantity, Cq, Tm_Target) %>%
  filter(str_starts(Sample, "STD"))

# Add column for log concentration and remove melt peak less than 75 degrees: 
data_std_curve_model <- data_std_curve_model %>% mutate(Log_conc = log10(Quantity)) %>% 
  filter(Tm_Target >= 75)

# Plot the data: 
plot1 <- ggplot(data = data_std_curve_model, aes(x=Log_conc, y=Cq))+
  geom_point()+
  geom_smooth(method = "lm", color = "blue")

plot1

# Through the Merkes et al (2019) code, we have already determined that the LOD is below 
# Standard 5, and that there is one outlier in Standard 1. So I will remove
# those two points from the plot 

# Remove the row where Sample is "STD-1" and Cq is 24.487333:
data_std_curve_model_clean <- data_std_curve_model[!(data_std_curve_model$Sample == "STD-1" & data_std_curve_model$Cq == 24.487333), ]

# Remove the values for STD-6: 
data_std_curve_model_clean <- data_std_curve_model_clean[!(data_std_curve_model_clean$Sample == "STD-6"), ]

# Re-plot the data: 
plot2 <- ggplot(data = data_std_curve_model_clean, aes(x=Log_conc, y=Cq))+
  geom_point()+
  geom_smooth(method="lm", colour = "blue")

plot2

# Running a linear regression on the standards: 
cor.test(~ Log_conc + Cq, data = data_std_curve_model_clean)
standard_lm <- lm(Cq ~ Log_conc, data = data_std_curve_model_clean)
coef(standard_lm)

Model_Intercept <- coef(standard_lm)[1]
Model_Slope <- coef(standard_lm)[2]

summary(standard_lm)


# Calculate assay efficiency ####

# Formula: E = -1+10(-1/slope)

E <- -1+10^(-1/Model_Slope)
print(E) # Assay efficiency is 103.4% (1dp)


# Plot the model #### 

plot_stdcurve.model <- ggplot(data_std_curve_model_clean, aes(x=Log_conc, y=Cq)) +
  geom_point(colour="#228886")+
  geom_smooth(method="lm", colour="#C45A12")+
  theme_bw()+
  xlab("Relative concentration of target DNA (log - base 10)")+
  ylab("Cq value") +
  annotate("text", x=4.5, y=27, label="y = -3.243x + 37.725")+
  annotate("text", x=4.5, y=26, label="Efficiency = 103.4%")

plot_stdcurve.model


# Final plot of model & samples ####

# Re-plotting the linear model using relative log (base 10) concentrations of target DNA:
plot_standard_lm <- ggplot() + 
  geom_abline(intercept = Model_Intercept, slope = Model_Slope, colour = "#228886") +
  xlim(0,6)+
  ylim(20,40)+
  xlab("Relative starting concentration of target DNA (log - base 10)") +
  ylab("Cq value") +
  theme_bw()+
  geom_vline(xintercept=log(current_LOD,10),colour="red") +
  geom_vline(xintercept=log(current_LOQ,10),linetype=2) +
  annotate("text", x = 0.35, y = 27, label = "LOD", angle=90, colour = "red", size = 4) +
  annotate("text", x = 0.35, y = 25, label = "LOQ", angle=90, colour = "black", size = 4) +
  annotate("text", x = 3.5, y = 21, label = "y = -3.243x + 37.725")+
  annotate("text", x = 3.25, y = 20, label = "Efficiency = 103.4%")
...
# Filtering dataset for columns of interest: 
data_all_wells_1 <- data_all_wells %>% dplyr::select(Sample, Quantity, Task, Cq, Tm_Target)

# Add column for log concentration and remove melt peaks < 75 degrees C: 
data_all_wells_1 <- data_all_wells_1 %>% mutate(Log_conc = log10(Quantity)) %>% 
  filter(Tm_Target >= 75)

# Add column for modelled log concentrations using y=mx+c from model and keeping
# original log conc values if they're not missing: 
data_all_wells_1 <- data_all_wells_1 %>% mutate(Log_conc_mod = ifelse(is.na(Log_conc),
                    (Cq - Model_Intercept) / Model_Slope, Log_conc))

# Removing outlier identified from previous steps:
# Remove the row where Sample is "STD-1" and Cq is 24.487333:
data_all_wells_clean <- data_all_wells_1[!(data_all_wells_1$Sample == "STD-1" & data_all_wells_1$Cq == 24.487333), ]

# Calculating average Cq of PCR replicates: 

data_all_wells_clean <- data_all_wells_clean %>% 
  group_by(Sample) %>%
  mutate(Av_Cq = mean(Cq, na.rm = TRUE)) %>%
  ungroup() 

# Add column for modelled log concentrations for the average Cq values: 
data_all_wells_clean <- data_all_wells_clean %>% 
  mutate(Log_conc_modAv = ifelse(Task == "Standard", Log_conc_mod, 
                                 ((Av_Cq - Model_Intercept) / Model_Slope)))

# Plot the resulting points (i.e. showing modelled log concentrations against 
# average Cq values across three PCR replicates)

plot_av <- ggplot(data_all_wells_clean, aes(x=Log_conc_modAv, y=Av_Cq, colour = Task))+
  geom_point()+
  theme_bw()

plot_av

# Define shapes for different sample types:
custom_shapes <- c("Standard" = 18, "Unknown" = 17)

# Add a column that assigns colours based on sample names:
data_all_wells_clean$colour_group <- ifelse(grepl("^APR", data_all_wells_clean$Sample), 
                                            "Positive Field Control",
                                     ifelse(grepl("^NEC", data_all_wells_clean$Sample), 
                                            "Negative Extraction Control",
                                     ifelse(grepl("^STD", data_all_wells_clean$Sample), 
                                            "Standard","eDNA water sample")))


# Add to linear model plot: 
plot_all_samples <- plot_standard_lm +
  geom_point(data=data_all_wells_clean, aes(x=Log_conc_modAv, y=Av_Cq, 
                                        colour = colour_group, 
                                        shape = Task), size=3) +
  scale_colour_manual(name = "Sample type", values = c("Positive Field Control"="#C45A12",
                                                      "Negative Extraction Control"="#A5A5A5",
                                                      "Standard"="black",
                                                      "eDNA water sample"="#33CCCC")) +
  scale_shape_manual(name = "PCR assay type", values = custom_shapes) +
  theme_bw()
